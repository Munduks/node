...
const udbName = process.env.UDB_NAME;
// const udbName = process.env.UDB_NAME;
...
app.get('/users', async (req, res) => {
  try {
    ...
    const data = await con.db('udbName').collection('users').find().toArray();
    await con.close();
    res.send(data);
  } catch (error) {
...
app.post('/users', async (req, res) => {
  try {
    ...
    const data = await con
      .db('manoDuomenuBaze')
      .collection('users')
      .insertOne(user); // prideda vieną objektą
    await con.close();
    res.send(data);
  } catch (error) {
    res.status(500).send(error);
...
//   foreignField: 'ownerId', // kitos kolekcijos raktas per kurį susijungia owner su pets kolekcija
...
//   }
// });
// // galima pasiziureti kas yra gyvunu, daiktu ar kazko ten savininkai
// app.get('/petsWithOwner', async (req, res) => {
//   try {
//     const con = await client.connect();
//     const data = await con
//       .db(dbName)
//       .collection('pets') // traukiama informacija is visu gyvunu
//       .aggregate([
//         {
//           $lookup: {
...
//             as: 'owner_info', // išeigos masyvo laukas,kur bus parodoma informacija
//           },
//         },
//         {
//           $unwind: '$owner_info', // išplečia masyvą, kad kiekvienas elementas būtų atskiras .
//           // vietoj padarymo i masyva padaro kaip objekta prideta info apie owner.viena savininka
//         },
//       ])
//       .toArray();
//     await con.close();
...
//   }
// });
// // naudojama kai istraukiamas id  is owneriu
// app.post('/pets', async (req, res) => {
//   // pries pridedant gyvunareikia prideti ju savininku kolekcija
//   try {
//     const con = await client.connect();
//     const data = await con
//       .db(dbName)
...
//           type: 'dog',
//           name: 'Madis',
//           ownerId: new ObjectId('645b4c503893c45e0bb836a0'), //  is jau esamu savininku owner
//         },
//       ]);
//     await con.close();
//     res.send(data);
